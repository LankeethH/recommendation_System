...
page = st.sidebar.radio("Select a page", ["Home","Rate", "History","Rated History"])
...
if page == "Rate":
 ...
 else:  st.text("Watch history is Empty")



elif page == "Rated History":
    st.header("Ratings_gaved")
    if len(st.session_state.Rated_Dict) != 0:
         
         for key, value in st.session_state.Rated_Dict.items():
            st.write(f"{key} - {value}")
    
    else:  st.text("NO earler ratings")
